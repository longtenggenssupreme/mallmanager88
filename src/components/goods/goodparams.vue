...
<el-card>
    ...
    <el-form class="form" label-position="left" label-width="80px" :model="form">
        <el-form-item label="商品分类">
            {{optionsselectarry}}
            <el-cascader clearable :show-all-levels="false" v-model="optionsselectarry" :options="options" :props="defaultporps" @change="handleChange">
            </el-cascader>
        </el-form-item>
    </el-form>
            ...
            <el-table :data="arrDynamicParams" style="width: 100%">
                <el-table-column type="expand" label="#" width="60">
                    <template slot-scope="scope">{{scope.row.att_name}}
                        <el-tag :key="tag" v-for="tag in scope.row.att_vals"
                        closable
